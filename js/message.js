...
Message.prototype.setMessageStatus = function(data) {
    ...
    if (status == "delivered") {
        message.status = "<i class='material-icons' style='font-size: 15px;margin-left: 10px;'>done_all</i>"                    
    }
    if (status == "seen") { 
        message.status = "<i class='material-icons' style='color:green;font-size: 15px;margin-left: 10px;'>done_all</i>" 
    } 
    if (status == "not delivered yet") { 
        message.status = "<i class='material-icons' style='font-size: 15px;margin-left: 10px;'>done</i>" 
    }
    if (status == "not sent") {
        message.status = "<i class='material-icons' style='font-size: 15px;margin-left: 10px;'>schedule</i>" 
    }
    ...
    if(dialogId === dialogModule.dialogId){
        ...
        if(messageElem !== undefined){
            ...
            $(".j-message__status").html(message.status);
            // statusElem.innerText = message.status;
        }
    }
};
